...
class Substation(BaseModel):
    """变电站定义"""
    name: str = Field(..., description="变电站名称")
    nodes: List[str] = Field(..., description="变电站包含的节点列表")
    switches: Dict[str, Switch] = Field(..., description="变电站包含的开关字典")
    zone_lines: Dict[str, ZoneLine] = Field(default_factory=dict, description="变电站包含的供区线路字典")
